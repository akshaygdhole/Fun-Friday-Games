...
export default function Categories() {
  ...
  const drawLetter = useCallback(() => {
    ...
    let g = 0;
    while (l === lastLetter && g++ < 32) l = ALPHABET[Math.floor(Math.random() * ALPHABET.length)];
    setLastLetter(l);
    return l;
  }, [lastLetter]);
  ...
  const drawCategory = useCallback(() => {
    ...
    let g = 0;
    while (i === lastCat && categories.length > 1 && g++ < 32) i = Math.floor(Math.random() * categories.length);
    setLastCat(i);
    return categories[i];
  }, [categories, lastCat]);
      ...
      <main className="activity-layout">
        ...
        <section className="panel activity-panel">
          ...
          <div className="activity-actions">
            ...
            <button type="button" className="btn btn-ghost" onClick={() => setRunning(true)} disabled={running}>
              Start 60s
            </button>
