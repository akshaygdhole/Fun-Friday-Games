...
export default function StickyScoreBar() {
  ...
  return (
    <header className="sticky-score-bar" role="region" aria-label="Live scores — quick adjust">
      <div className="sticky-score-bar-inner">
        ...
        <div className="sticky-score-bar-teams">
          <div className="sticky-score-team">
            ...
            <div className="sticky-score-quick">
              <button type="button" className="sticky-score-btn sticky-score-btn-plus" onClick={() => addScore(0, 1)} title={`${a.name}: +1`} aria-label={`${a.name} plus one`}>
                +1
              </button>
              <button type="button" className="sticky-score-btn sticky-score-btn-minus" onClick={() => addScore(0, -1)} title={`${a.name}: foul`} aria-label={`${a.name} minus one`}>
                −1
              </button>
            </div>
          </div>
          ...
          <div className="sticky-score-team">
            ...
            <div className="sticky-score-quick">
              <button type="button" className="sticky-score-btn sticky-score-btn-plus" onClick={() => addScore(1, 1)} title={`${b.name}: +1`} aria-label={`${b.name} plus one`}>
                +1
              </button>
              <button type="button" className="sticky-score-btn sticky-score-btn-minus" onClick={() => addScore(1, -1)} title={`${b.name}: foul`} aria-label={`${b.name} minus one`}>
                −1
              </button>
            </div>
          </div>
        ...
        <div className="sticky-score-bar-actions">
          <Link to="/scoreboard" className="sticky-score-link sticky-score-link-primary">
            Full scoreboard
          </Link>
